我们已经设计并实现了GFS，一个为大型分布式密集应用设计的弹性分布式文件系统。

在普通的硬件上运行时，他提供了错误容忍，且在高并发下仍有高性能。

虽然和以前很多分布式文件系统目标一样，我们的系统是由近期和可预见的未来下应用的负载和技术环境驱动设计的。

# 介绍
## 错误
我们已经设计并实现了GFS以应对数据处理程序快速增长的需求

GFS实现了和以前的分布式文件系统相同的目标，例如：性能、伸缩性、 可靠性、可用性。

这套系统是由应用程序驱动设计，在某些设计上和以前的系统有所不同，例如我们认为节点错误是必然发生的，而不是个异常，机器故障、人工错误、进程错误、网络中断都可能引起单节点错误，我们承认这些错误的存在，并监控节点，自动让错误节点恢复

## 文件
文件通常是很大的，几个G的文件是很正常的。一个典型的文件包含了很多应用对象，比如说web文档

我们通常将快速增长的T级别数据解析为数百万个小对象，即使系统支持，一般也不会操作几百万KB级别的文件，所以不得不重新考虑假设和参数的设计，例如IO操作和文件块大小

文件通常都是追加数据，而不是更改数据，基本不存在随机读取文件中内容的操作，一旦编写，文件只能读取，而且是顺序读取。各种各样的数据，都有这样的特征

一些可能是数据分析程序用来扫描的知识库，一些可能是流式写入程序，一些可能是档案数据，一些可能是中间变量

# 总览
## 假设
我们通过观察应用的使用情况提出了一些假设

1. 机器是由许多便宜普通的主机组成，时常宕机。必须监控并检测他们，容忍宕机存在且让节点快速恢复
2. 系统存放适量多的大文件，我们假设是几百万个文件(如果一个文件结构用1k表示，1k * (x 000 000) 大概几个g能存下所有的文件节点)，通常任务文件在100M左右，几个G的也可以，并不对文件大小做限制 
3. 应用操作主要考虑两个读：大的流式读，小的随机读
    1. 流式读时典型的读取是几百K到1M，每次读取成功后会继续读下去
    2. 小的随机读典型的是在一个随意的偏移量下读几k，
4. 应用会有大的序列化的对文件追加数据的操作，典型的操作尺寸和读相似，写入之后文件一般就不改了。随机写也支持，但是效率低
5. 系统要高效的实现并发读写，这些文件常被用于生产消费者队列，在很多消费者或生产者读写的条件下尽可能降低锁的粒度
6. 高速度比低延迟更重要，应用只注重数据传输速度，不在乎延迟

## 接口
GFS实现了一个非标准IO的文件系统的接口，文件像linux文件系统分级存储，每个文件都可以通过路径唯一标识，提供了这些操作：create, delete, open, close, read, write

GFS提供了快照和插入记录。快照以很低的小号创建文件或文件夹的备份。插入记录可以保证多client并发写入同一文件操作原子性，它对多路写入和消费生产者模式很有用的

一个GFS集群由多种服务器节点组成，并且被多个client使用，这些节点基本都是运行着client进程的普通的linux机器。他们可以同时运行文件系统进程和client进程

文件被切割为固定大小的chunk，在chunk被创造时，主机会为每个chunk生成一个不会变化的、全局唯一的64b chunk handle标识(分割成块可以减少并发冲突，且负载均衡，每个块生成唯一标识可以快速定位查找)。默认会存储三个chunk副本，用户也可以为文件命名空间指定不同的备份级别(多备份防止单机故障)

master节点维护文件系统的元信息，包括命名空间，访问控制信息(并发控制)，file chunk的映射，和最近位置的chunk。它协调整个系统运作，比如：chunk租借管理、收集孤儿chunk、在chunkserver之间移动chunk，master周期性的向chunkserver发送心跳包，下发指令并收集chunkserver信息

和每个应用相关联的GFS client实现了文件系统的API，并且代表应用和master和chunkserve通讯实现读写数据。client和master进行元数据操作交互，但是所有数据是从chunkserve访问到的

![](img/1.png)

我们不打算提供POSIX API，因此不需要挂钩子到vnode节点

client和chunkserver都不会缓存文件数据，client缓存数据的意义很小，因为大部分应用流式读取文件或者需要缓存太多，没有缓存简化了客户端和消除了额整个系统一致性问题

## 单一master节点
用单一的master节点能够很简化我们的设计
